...
int ll_remove_sequence(struct linked_list_t *ll, const struct linked_list_t *ll_2) {
    if (ll == NULL || ll_2 == NULL)
        return -1;
    const struct linked_list_t *ll2 = ll_2;
    int s = 0;
    int s2 = ll_size(ll2);
    if(s2==0)
        return 0;
    int l = 0, err, err2, f = 0;

    for (int i = 0; i < ll_size(ll);) {
        if (ll_at(ll, i, NULL) == ll_at(ll2, 0, NULL)) {
            l = 0;

            while (ll_at(ll, i + l, &err) == ll_at(ll2, l, &err2)) {
                if (err != 0 || err2 != 0)
                    break;
                l++;
            }
