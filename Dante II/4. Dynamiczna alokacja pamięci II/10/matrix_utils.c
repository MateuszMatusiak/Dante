...
struct matrix_t *matrix_copy(const struct matrix_t *copy) {
    if (copy == NULL)
        return NULL;
    if (copy->height <= 0 || copy->width <= 0 || copy->ptr == NULL)
        return NULL;
    struct matrix_t *xd = matrix_create_struct(copy->width, copy->height);
    if (xd == NULL)
        return NULL;
    for (int i = 0; i < copy->height; ++i) {
        for (int k = 0; k < copy->width; ++k) {
            *(*(xd->ptr + i) + k) = *(*(copy->ptr + i) + k);
        }
    }
    return xd;
}
...
struct matrix_t *matrix_load_b(const char *filename, int *err_code) {
    ...
    struct matrix_t *xd = matrix_create_struct(w, h);
    if (xd == NULL) {
        if (err_code != NULL)
            *err_code = 4;
        fclose(f);
    ...
    for (int i = 0; i < h; ++i) {
        for (int k = 0; k < w; ++k) {
            t = fread((*(xd->ptr + i) + k), sizeof(int), 1, f);
            if (t != 1) {
                if (err_code != NULL)
                    *err_code = 3;
                matrix_destroy_struct(&xd);
                fclose(f);
                return NULL;
            }
        }
...
struct matrix_t *matrix_add(const struct matrix_t *m1, const struct matrix_t *m2) {
    if (m1 == NULL||m2==NULL)
        return NULL;
    if (m1->height <= 0 || m1->width <= 0 || m1->ptr == NULL||m2->height <= 0 || m2->width <= 0 || m2->ptr == NULL||m1->height!=m2->height||m1->width!=m2->width)
        return NULL;
    struct matrix_t *xd = matrix_create_struct(m1->width, m1->height);
    if (xd == NULL)
        return NULL;

    for (int i = 0; i < m1->height; ++i) {
        for (int k = 0; k < m1->width; ++k) {
            *(*(xd->ptr+i)+k)=*(*(m1->ptr+i)+k)+*(*(m2->ptr+i)+k);
        }
    }
...
struct matrix_t *matrix_multiply(const struct matrix_t *m1, const struct matrix_t *m2) {
    if (m1 == NULL||m2==NULL)
        return NULL;
    if (m1->height <= 0 || m1->width <= 0 || m1->ptr == NULL||m2->height <= 0 || m2->width <= 0 || m2->ptr == NULL||m1->height!=m2->width||m1->width!=m2->height)
        return NULL;
    struct matrix_t *xd = matrix_create_struct(m2->width, m1->height);
    if (xd == NULL)
        return NULL;
    for(int i=0;i<m1->height;++i)
    {
        ...
        {
            *(*(xd->ptr+i)+k)=0;
            for(int o=0;o<m2->height;++o)
                *(*(xd->ptr+i)+k)=*(*(xd->ptr+i)+k)+*(*(m1->ptr+i)+o)**(*(m2->ptr+o)+k);
        }
    }
    return xd;
}
