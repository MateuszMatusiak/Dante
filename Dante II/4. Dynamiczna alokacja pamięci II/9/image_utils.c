...
int image_draw_rectangle(struct image_t *img, const struct rectangle_t *rect, int value) {
    if (rect == NULL)
        return 1;
    if (img == NULL || img->height <= 0 || img->width <= 0 || img->ptr == NULL || strcmp(img->type, "P2") != 0 ||
        rect->p.x < 0 || rect->p.y < 0 || rect->p.x > img->width - rect->width ||
        rect->p.y > img->height - rect->height || rect->width <= 0 || rect->height <= 0 || value < 0 || value > 255) {
        return 1;
    }
...
int draw_image(struct image_t *img, const struct image_t *src, int destx, int desty) {
    if (img == NULL || img->height <= 0 || img->width <= 0 || img->ptr == NULL || strcmp(img->type, "P2") != 0 || src == NULL || src->height <= 0 || src->width <= 0 || src->ptr == NULL || strcmp(src->type, "P2") != 0 || destx < 0 || desty < 0 || destx > img->width - src->width || desty > img->height - src->height) {
        return 1;
    }
    int x=0,y=0;
    ...
    {
        x=0;
        for(int k=destx;k<destx+src->width;++k)
        {
            *(*(img->ptr+i)+k)=*(*(src->ptr+y)+x);
            ++x;
        }
        ++y;
    }
